...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation, visualize_dir=None) -> Optional[STEP_OUTPUT]:
        images, labels = batch
        ...
        preds, probs = self.tokenizer.decode(probs)
        # print(f"Preds: {preds}, \nProbs: {probs}\n Labels: {labels}")
        for image, pred, prob, gt in zip(images, preds, probs, labels):
            ...
            if visualize_dir is not None:
                ...
                image_vis = torch.stack((b,g,r),dim=0)*0.5+0.5
                image_vis = image_vis.permute(1,2,0).cpu().numpy()
                image_vis = (image_vis*255).astype(np.uint8)
                flag = str(int(pred == gt))
                name = gt + '_' + flag + '_' + pred + '.jpg'
                img_path = os.path.join(visualize_dir, name)
                if os.path.exists(img_path):
                    i=0
                    while os.path.exists(img_path):
                        name = gt + '_' + flag + '_' + pred + '_'+str(i)+'.jpg'
                        img_path = os.path.join(visualize_dir, name)
                        i += 1
                cv2.imwrite(img_path, image_vis)
    ...
    def test_step(self, batch, batch_idx, visualize_dir=None) -> Optional[STEP_OUTPUT]:
        return self._eval_step(batch, False, visualize_dir=visualize_dir)
