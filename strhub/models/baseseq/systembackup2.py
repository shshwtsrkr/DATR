...
class BaseSeq(CrossEntropySystem):
    ...
    def decode(self, tgt: torch.Tensor, memory: torch.Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        tgt = tgt.long()  # Cast to Long if it's not already

        # Now, proceed with embedding lookup as before
...
class FeatureCombiner(nn.Module):
    def __init__(self, primary_feat_dim, helper_feat_dim, output_dim):
        super(FeatureCombiner, self).__init__()
        # Assuming primary_feat_dim == helper_feat_dim == output_dim for clip_embed
        self.transform = nn.Sequential(
            nn.Linear(primary_feat_dim + helper_feat_dim, output_dim),
            nn.ReLU(),  # Activation function can be adjusted as needed
            # Further layers can be added as needed for more complex transformations
        )
    
    def forward(self, primary_feat, helper_feat):
        # Concatenate features along the feature dimension (dim=1)
        combined = torch.cat((primary_feat, helper_feat), dim=1)
        # Transform back to the original dimensionality
        transformed = self.transform(combined)
        return transformed

class TgtFeatureCombiner(nn.Module):
    def __init__(self, primary_feat_dim, helper_feat_dim, output_dim):
        super(TgtFeatureCombiner, self).__init__()
        self.transform = nn.Sequential(
            nn.Linear(primary_feat_dim + helper_feat_dim, output_dim),
            nn.ReLU(),
        )
...
class CLIPOCR(BaseSeq):
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        primary_feat_dim = primary_clip_embed.shape[-1]  # For clip_embed
        helper_feat_dim = helper_clip_embed.shape[-1]   # For clip_embed
        output_dim = primary_clip_embed.shape[-1]  # Desired output dimension to match primary features

        combiner = FeatureCombiner(primary_feat_dim, helper_feat_dim, output_dim).to('cuda')

        clip_embed = combiner(primary_clip_embed, helper_clip_embed)

        print(f"clip_embed type: {type(clip_embed)}, shape/length: {len(clip_embed) if isinstance(clip_embed, list) else clip_embed.shape}")

        # primary_tgt_dim = primary_tgt.shape[-1]
        # helper_tgt_dim = helper_tgt.shape[-1]
        # output_dim = primary_tgt.shape[-1]  # Keep the primary feature's dimension

        # tgt_combiner = TgtFeatureCombiner(primary_tgt_dim, helper_tgt_dim, output_dim).to('cuda')
        # primary_tgt = primary_tgt.to(torch.float16)
        # helper_tgt = helper_tgt.to(torch.float16)

        # tgt = tgt_combiner(primary_tgt, helper_tgt)

        print(f"tgt type: {type(tgt)}, shape/length: {len(tgt) if isinstance(tgt, list) else tgt.shape}")
        ...
        print(f"memory_list type: {type(memory_list)}, shape/length: {len(memory_list) if isinstance(memory_list, list) else memory_list.shape}")
        ...
        for i, (seq1, seq2) in enumerate(zip(out_list[:-1], f_list)):
            ...
            print(f"Sequence 1 f_list type: {type(seq1)}, shape/length: {len(seq1) if isinstance(seq1, list) else seq1.shape}")
            print(f"Sequence 2 f_list type: {type(seq2)}, shape/length: {len(seq2) if isinstance(seq2, list) else seq2.shape}")
            seq1_len, seq2_len = seq1.shape[0], seq2.shape[0]
            if seq1_len > seq2_len:
                # Pad seq2 to match seq1's length
                padding = seq1_len - seq2_len
                seq2 = F.pad(seq2, (0, 0, 0, padding), "constant", 0)
            elif seq2_len > seq1_len:
                # Pad seq1 to match seq2's length
        ...
        for i, (seq1, seq2) in enumerate(zip(memory_list[:-1], f_list_vis)):
            ...
            print(f"Sequence 1 f_list_vis type: {type(seq1)}, shape/length: {len(seq1) if isinstance(seq1, list) else seq1.shape}")
            print(f"Sequence 2 f_list_vis type: {type(seq2)}, shape/length: {len(seq2) if isinstance(seq2, list) else seq2.shape}")
            seq1_len, seq2_len = seq1.shape[0], seq2.shape[0]
            if seq1_len > seq2_len:
                # Pad seq2 to match seq1's length
                padding = seq1_len - seq2_len
                seq2 = F.pad(seq2, (0, 0, 0, padding), "constant", 0)  # Pad the last dimension if needed
            elif seq2_len > seq1_len:
                # Pad seq1 to match seq2's length
                padding = seq2_len - seq1_len
                seq1 = F.pad(seq1, (0, 0, 0, padding), "constant", 0)  # Pad the last dimension if needed
